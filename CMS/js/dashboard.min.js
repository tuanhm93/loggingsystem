...
var urlService = 'http://localhost:3000/api';
...
angular.module("RDash").controller('DashboardCtrl', function($scope, $http) {
    ...
    $scope.appInf.breadcrumb = 'home/dashboard';
	 ...
	 var getNumberApps = function(){
        $http.get(urlService+'/app')
            .then(function(data){
                var data = data.data;
                if(data.success){
                    $scope.loadApps = false;
                    $scope.apps = data.listApp.length; 
                }
            });
    }
...
angular.module("RDash").controller('ListAppsCtrl', function($scope, $http, $window) {
    ...
    $scope.delete = function(index){
    	...
    	if(confirm){
    		$http.delete(urlService+'/app/'+$scope.listApps[index]._id)
	    		.then(function(data){
...
angular.module("RDash").controller('ModifyAppCtrl', function($scope, $http, $stateParams) {
    ...
    $http.get(urlService+'/app/'+$stateParams.token)
    	.success(function(data){
    		console.log(data);
    		if(data.success){
    			$scope.old = data.app.expireTime;
    			$scope.app = data.app;
    		}
    	});
    ...
    $scope.update = function(){
    	...
    	$scope.button = false;
    	$http.put(urlService+'/app/'+$stateParams.token, {expireTime: $scope.app.expireTime})
    		.then(function(data){
    			if(data.data.success){
...
angular.module("RDash").controller('CreateAppCtrl', function($scope, $http) {
    ...
    $scope.create = function(){
    	...
    	$scope.faild = false;
    	if($scope.app.name != ''){
    		$http.post(urlService+'/app', {name: $scope.app.name})
    			.then(function(data){
    				if(data.data.success){
...
angular.module("RDash").controller('WorkerCtrl', function($scope, $http) {
    ...
    $scope.change = function(){
    	...
    	$scope.success = false;
    	$scope.faild = false;
    	$http.post(urlService+"/worker", {numWorkers: $scope.numberWorkers})
    		.then(function(data){
                console.log('haha');
    			if(data.data.success){
    				$scope.success = true;
